...
class MotorThread(threading.Thread):
    def __init__(self):
        ...
        #Front motor is connected to output A (!WORK IN PROGRESS!)
    ...
    def run(self):
        ...
        while running:
            ...
            #Front motor is still a work in progress
            #self.front_motor.run_forever(speed_sp=dc_clalmp(forward_speed))
...
for event in gamepad.read_loop():   #This loops infinitely when running = True
    ...
    if event.type == 1 and event.code == 305 and event.value == 1:
        print("X button is pressed. Stopping.") #Press X button to turn of engine
        running = False
        time.sleep(0.5) #Wait for the motor thread to finish
        break 

#Buttons to reference afterwards
#X 305
#O 306
#Triangle 307
#Square 304
